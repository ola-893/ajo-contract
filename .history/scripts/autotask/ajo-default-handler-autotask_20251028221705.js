...
async function isGasPriceAcceptable(provider) {
  ...
  if (gasPriceGwei > CONFIG.MAX_GAS_PRICE_GWEI) {
    console.log(`⚠️  Gas price too high (max: ${CONFIG.MAX_GAS_PRICE_GWEI} gwei)`);
    return false;
  }
...
async function processAjoDefaults(ajoCore, ajoPayments, ajoId, signer) {
  ...
  try {
    ...
    const [isPastDeadline, defaultersCount, defaulters, currentCycle, deadline] = status;
    ...
    if (!isPastDeadline) {
      ...
      return { ajoId, success: true, message: 'Deadline not reached', processed: 0 };
    }
    
    if (defaultersCount.toNumber() === 0) {
      console.log(`   ✅ No action needed - all members paid`);
      return { ajoId, success: true, message: 'All members paid', processed: 0 };
    }
    ...
    const event = receipt.events?.find(e => e.event === 'DefaultsHandledByAutomation');
...
exports.handler = async function(event) {
  ...
  try {
    ...
    for (let i = 0; i < statuses.length; i++) {
      ...
      if (status.enabled && status.shouldRun && status.defaultersCount.toNumber() > 0) {
    ...
    if (ajosThisRun.length < ajosToProcess.length) {
      console.log(`⚠️  Limiting to ${ajosThisRun.length} Ajos this run (${ajosToProcess.length - ajosThisRun.length} remaining)`);
    }
    ...
    for (const ajoId of ajosThisRun) {
      try {
        ...
        const ajoCore = new ethers.Contract(ajoCoreAddress, AJO_CORE_ABI, provider);
        const ajoPayments = new ethers.Contract(ajoPaymentsAddress, AJO_PAYMENTS_ABI, provider);
        
        // Process defaults
        const result = await processAjoDefaults(ajoCore, ajoPayments, ajoId.toNumber(), signer);
        results.push(result);
    ...
    const summary = {
      ...
      totalDefaultsProcessed: results.reduce((sum, r) => sum + (r.processed || 0), 0),
