...
async function testProposalCreation(ajoGovernance, members, ajoInfo) {
  ...
  try {
    const tx1 = await ajoGovernance.connect(members[0].signer).proposeCycleCompletion(
      "Complete Cycle 1 and prepare for next cycle",
      { gasLimit: 500000 }
    );
...
async function testCycleStatus(ajoGovernance) {
  ...
  try {
    ...
    console.log(c.dim("     ┌────────────────────────────────────────────────────┐"));
    console.log(c.dim(`     │ Current Cycle:           ${cycleStatus._currentCycle.toString().padEnd(24)} │`));
    console.log(c.dim(`     │ Cycle Completed:         ${(cycleStatus._isCycleCompleted ? 'Yes' : 'No').padEnd(24)} │`));
    console.log(c.dim(`     │ Participation Deadline:  ${cycleStatus._participationDeadline.toString().padEnd(24)} │`));
    console.log(c.dim(`     │ Declared Participants:   ${cycleStatus._declaredParticipants.toString().padEnd(24)} │`));
    console.log(c.dim("     └────────────────────────────────────────────────────┘\n"));
    ...
    console.log(c.red(`     ❌ Failed to get cycle status: ${error.message.slice(0, 100)}\n`));
    throw error;
  }
...
async function testParticipationDeclaration(ajoGovernance, members) {
  ...
  for (let i = 0; i < Math.min(5, members.length); i++) {
    ...
    try {
      const tx = await ajoGovernance.connect(member.signer).declareNextCycleParticipation(
        willParticipate,
        { gasLimit: 200000 }
      );
...
async function runGovernanceDemo(ajoGovernance, participantsInput, ajoInfo) {
  ...
  console.log(c.dim("     Votes Cast (Real HCS):     ") + c.green(votes.length.toString()));
  ...
  console.log(c.dim("     • Full cycle management - members can opt-in/opt-out"));
  ...
  if (hederaTopicId) {
    console.log(c.dim(`        curl https://testnet.mirrornode.hedera.com/api/v1/topics/${hederaTopicId}/messages`));
  }
