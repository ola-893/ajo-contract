...
async function runGovernanceDemo(ajoGovernance, participantsInput, ajoInfo) {
  ...
  for (let i = 0; i < Math.min(5, members.length); i++) {
    console.log(
      c.dim(
        `     ${i + 1}. ${members[i].name} (${members[i].address.slice(
          0,
          10
        )}...)`
      )
  ...
  if (tallyResult) {
    ...
    await validateContractState(ajoGovernance, proposalId, {
      ...
      voters: votes.map((v) => ({
        address: v.voter,
        name:
          members.find((m) => m.address.toLowerCase() === v.voter.toLowerCase())
            ?.name || "Unknown",
        expectedVoted: true,
      })),
    });
  ...
  if (declarations.length > 0) {
    const expectedDeclared = declarations.filter(
      (d) => d.willParticipate
    ).length;
    await validateSeasonState(ajoGovernance, {
      expectedSeason: 1,
  ...
  console.log(
    c.dim("     Votes Cast (HCS):          ") + c.green(votes.length.toString())
  );
  console.log(
  ...
  console.log(
    c.dim("     • State validated on both HCS Mirror Node and smart contract")
  );
  ...
  if (hederaTopicId) {
    console.log(
      c.dim(
        `        curl https://${network}.mirrornode.hedera.com/api/v1/topics/${hederaTopicId}/messages`
      )
    );
  }
  console.log(
  ...
  console.log(
    c.dim("     4. Frontend integration: Use validated states for UI updates\n")
  );
