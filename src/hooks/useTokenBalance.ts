...
export const useTokenBalance = (tokenSymbol: "STRK" | "USDC" | "ETH") => {
  ...
  const fetchBalance = async () => {
    ...
    try {
      // Try to get provider from wallet account, fallback to creating one
      let provider: ProviderInterface;
      
      try {
        // Try to use the wallet's provider
        provider = (account as any).provider as ProviderInterface;
        if (!provider) {
          throw new Error("Wallet provider not found");
        }
      } catch (e) {
        // Fallback to creating a new RpcProvider with Alchemy API key
        console.log("Creating new RPC provider for balance fetching");
        provider = new RpcProvider({
          nodeUrl: "https://starknet-sepolia.g.alchemy.com/v2/W7Jx4ZJo0o9FaoLXaNRG4"
        });
      }
