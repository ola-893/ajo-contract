...
async function demoFactoryViewFunctions(ajoFactory, deployer) {
  ...
  try {
    ...
    if (stats.success) {
      console.log(
        c.dim(
          `       Total: ${stats.data.totalCreated}, Active: ${stats.data.activeCount}`
        )
      );
    }
    ...
    if (impls.success) {
      console.log(
        c.dim(`       AjoCore: ${impls.data.ajoCore.slice(0, 10)}...`)
      );
      console.log(
        c.dim(`       AjoMembers: ${impls.data.ajoMembers.slice(0, 10)}...`)
      );
      console.log(
        c.dim(
          `       AjoCollateral: ${impls.data.ajoCollateral.slice(0, 10)}...`
        )
      );
      console.log(
        c.dim(`       AjoPayments: ${impls.data.ajoPayments.slice(0, 10)}...`)
      );
      console.log(
        c.dim(
          `       AjoGovernance: ${impls.data.ajoGovernance.slice(0, 10)}...`
        )
      );
    }
    ...
    if (allAjos.success) {
      console.log(
        c.dim(
          `       Retrieved: ${allAjos.data.ajoInfos.length} Ajos, HasMore: ${allAjos.data.hasMore}`
        )
      );
    }
    ...
    if (stats.success && stats.data.totalCreated.gt(0)) {
      ...
      if (ajoInfo.success) {
        console.log(
          c.dim(
            `       Name: ${ajoInfo.data.name}, Active: ${ajoInfo.data.isActive}`
          )
        );
      }
      ...
      if (ajoCore.success) {
        console.log(
          c.dim(`       AjoCore Address: ${ajoCore.data.slice(0, 10)}...`)
        );
      }
      ...
      if (status.success) {
        console.log(
          c.dim(
            `       Exists: ${status.data.exists}, Active: ${status.data.isActive}`
          )
        );
      }
      ...
      if (initStatus.success) {
        console.log(
          c.dim(
            `       Phase: ${initStatus.data.phase}, Ready: ${initStatus.data.isReady}`
          )
        );
      }
    }
...
async function demoMemberViewFunctions(ajo, ajoMembers, participants) {
  ...
  try {
    ...
    if (activeList.success) {
      console.log(
        c.dim(`       Active Members Count: ${activeList.data.length}`)
      );
    ...
    for (let i = 0; i < Math.min(3, participants.length); i++) {
      ...
      if (member.success) {
        console.log(
          c.dim(
            `       Queue#: ${member.data.queueNumber}, Active: ${member.data.isActive}`
          )
        );
        console.log(
          c.dim(
            `       Locked Collateral: ${formatUSDC(
              member.data.lockedCollateral
            )} USDC`
          )
        );
        console.log(
          c.dim(`       Total Paid: ${formatUSDC(member.data.totalPaid)} USDC`)
        );
      ...
      if (memberInfo.success) {
        console.log(
          c.dim(
            `       Pending Penalty: ${formatUSDC(
              memberInfo.data.pendingPenalty
            )} USDC`
          )
        );
        console.log(
          c.dim(
            `       Voting Power: ${ethers.utils.formatEther(
              memberInfo.data.effectiveVotingPower
            )}`
          )
        );
      }
      ...
      if (queueInfo.success) {
        ...
        console.log(
          c.dim(
            `       Est. Wait Cycles: ${queueInfo.data.estimatedCyclesWait}`
          )
        );
      }
      ...
      if (lockedColl.success) {
        console.log(
          c.dim(
            `       Locked Collateral Direct: ${formatUSDC(
              lockedColl.data
            )} USDC`
          )
        );
    ...
    for (let pos = 1; pos <= Math.min(5, participants.length); pos++) {
      ...
      if (member.success) {
        console.log(
          c.dim(`       Position ${pos}: ${member.data.slice(0, 10)}...`)
        );
      }
      ...
      if (guarantor.success) {
        console.log(
          c.dim(
            `       Guarantor for ${pos}: ${guarantor.data.slice(0, 10)}...`
          )
        );
      }
    }
    ...
    for (let i = 0; i < Math.min(3, participants.length); i++) {
      ...
      if (memberAtIndex.success) {
        console.log(
          c.dim(`       Index ${i}: ${memberAtIndex.data.slice(0, 10)}...`)
        );
      }
    }
    ...
    if (stats.success) {
      ...
      console.log(
        c.dim(
          `       Current Queue Position: ${stats.data.currentQueuePosition}`
        )
      );
    }
  } catch (error) {
...
async function demoPaymentViewFunctions(ajo, ajoPayments, participants) {
  ...
  try {
    ...
    if (activeToken.success) {
      console.log(
        c.dim(
          `       Active Token: ${activeToken.data === 0 ? "USDC" : "HBAR"}`
        )
      );
    }
    ...
    if (penaltyRate.success) {
      console.log(
        c.dim(
          `       Penalty Rate: ${ethers.utils.formatUnits(
            penaltyRate.data,
            16
          )}%`
        )
    ...
    if (usdcConfig.success) {
      console.log(
        c.dim(
          `       USDC Monthly: ${formatUSDC(
            usdcConfig.data.monthlyPayment
          )}, Active: ${usdcConfig.data.isActive}`
        )
      );
    }
    ...
    if (hbarConfig.success) {
      console.log(
        c.dim(
          `       HBAR Monthly: ${formatHBAR(
            hbarConfig.data.monthlyPayment
          )} HBAR, Active: ${hbarConfig.data.isActive}`
        )
      );
    }
    ...
    if (calcPayout.success) {
      console.log(
        c.dim(`       Calculated Payout: ${formatUSDC(calcPayout.data)}`)
      );
    }
    ...
    if (nextRecip.success) {
      console.log(
        c.dim(`       Next Recipient: ${nextRecip.data.slice(0, 10)}...`)
      );
    }
          ...
          if (payout.success) {
            console.log(
              c.dim(
                `       Payout ${i}: ${formatUSDC(
                  payout.data.amount
                )} to ${payout.data.recipient.slice(0, 10)}... (Cycle ${
                  payout.data.cycle
                })`
              )
            );
    ...
    for (let i = 0; i < Math.min(3, participants.length); i++) {
      ...
      if (penalty.success) {
        console.log(
          c.dim(`       ${p.name} Pending Penalty: ${formatUSDC(penalty.data)}`)
        );
      }
    }
  } catch (error) {
...
async function demoCollateralViewFunctions(ajoCollateral, participants) {
  ...
  try {
    ...
    if (totalColl.success) {
      console.log(
        c.dim(`       Total USDC: ${formatUSDC(totalColl.data.totalUSDC)}`)
      );
      console.log(
        c.dim(`       Total HBAR: ${formatHBAR(totalColl.data.totalHBAR)}`)
      );
    }
    ...
    for (const scenario of scenarios) {
      const required = await safeViewCall(
        async () =>
          await ajoCollateral.calculateRequiredCollateral(
            scenario.pos,
      ...
      if (required.success) {
        console.log(
          c.dim(`       ${scenario.desc}: ${formatUSDC(required.data)} USDC`)
        );
      }
    }
    ...
    for (let pos = 1; pos <= Math.min(5, 10); pos++) {
      ...
      if (guarantorPos.success) {
        console.log(
          c.dim(
            `       Position ${pos} → Guarantor Position ${guarantorPos.data}`
          )
        );
      }
    }
    ...
    for (let i = 0; i < Math.min(3, participants.length); i++) {
      ...
      if (seizable.success) {
        ...
        console.log(
          c.dim(
            `         Total Seizable: ${formatUSDC(
              seizable.data.totalSeizable
            )}`
          )
        );
        console.log(
          c.dim(
            `         From Collateral: ${formatUSDC(
              seizable.data.collateralSeized
            )}`
          )
        );
        console.log(
          c.dim(
            `         From Payments: ${formatUSDC(
              seizable.data.paymentsSeized
            )}`
          )
        );
      }
...
async function demoCoreViewFunctions(ajo, participants) {
  ...
  try {
    ...
    if (stats.success) {
      ...
      console.log(
        c.dim(
          `       Total Collateral USDC: ${formatUSDC(
            stats.data.totalCollateralUSDC
          )}`
        )
      );
      console.log(
        c.dim(
          `       Total Collateral HBAR: ${formatHBAR(
            stats.data.totalCollateralHBAR
          )}`
        )
      );
      console.log(
        c.dim(
          `       Contract Balance USDC: ${formatUSDC(
            stats.data.contractBalanceUSDC
          )}`
        )
      );
      console.log(
        c.dim(
          `       Contract Balance HBAR: ${formatHBAR(
            stats.data.contractBalanceHBAR
          )}`
        )
      );
      console.log(
      ...
      console.log(
        c.dim(
          `       Active Token: ${
            stats.data.activeToken === 0 ? "USDC" : "HBAR"
          }`
        )
      );
    }
    ...
    if (usdcConfig.success) {
      console.log(
        c.dim(
          `       USDC Config - Monthly: ${formatUSDC(
            usdcConfig.data.monthlyPayment
          )}, Active: ${usdcConfig.data.isActive}`
        )
      );
    ...
    const demo = await safeViewCall(
      async () =>
        await ajo.getCollateralDemo(10, ethers.utils.parseUnits("50", 6)),
    ...
    if (demo.success) {
      ...
      for (let i = 0; i < Math.min(5, demo.data.positions.length); i++) {
        console.log(
          c.dim(
            `       Position ${demo.data.positions[i]}: ${formatUSDC(
              demo.data.collaterals[i]
            )} USDC`
          )
        );
      }
    ...
    for (let i = 0; i < Math.min(3, participants.length); i++) {
      ...
      if (memberInfo.success) {
        console.log(c.dim(`       ${p.name}:`));
        console.log(
          c.dim(`         Queue: ${memberInfo.data.memberInfo.queueNumber}`)
        );
        console.log(
          c.dim(
            `         Locked: ${formatUSDC(
              memberInfo.data.memberInfo.lockedCollateral
            )}`
          )
        );
        console.log(
      ...
      if (queueInfo.success) {
        console.log(
          c.dim(
            `         Position: ${queueInfo.data.position}, Wait: ${queueInfo.data.estimatedCyclesWait} cycles`
          )
        );
      }
...
async function demoGovernanceViewFunctions(ajoFactory, ajoId, participants) {
  ...
  try {
    ...
    if (settings.success) {
      console.log(
        c.dim(
          `       Proposal Threshold: ${ethers.utils.formatEther(
            settings.data.proposalThreshold
          )}`
        )
      );
      console.log(
        c.dim(`       Voting Period: ${settings.data.votingPeriod} seconds`)
      );
      console.log(
        c.dim(
          `       Penalty Rate: ${ethers.utils.formatUnits(
            settings.data.currentPenaltyRate,
            16
          )}%`
        )
      );
      console.log(
        c.dim(`       Total Proposals: ${settings.data.totalProposals}`)
      );
    }
    ...
    try {
      ...
      if (proposalEvent) {
        ...
        console.log(
          c.green(`      ✅ Test proposal created: ID ${proposalId}`)
        );
        ...
        if (proposal.success) {
          console.log(
            c.dim(`         Description: ${proposal.data.description}`)
          );
          console.log(
            c.dim(
          ...
          console.log(
            c.dim(
              `         Against: ${ethers.utils.formatEther(
          ...
          console.log(
            c.dim(
              `         Abstain: ${ethers.utils.formatEther(
                proposal.data.abstainVotes
              )}`
            )
          );
        ...
        for (let i = 0; i < Math.min(3, participants.length); i++) {
          ...
          try {
            const voteTx = await governance
              .connect(p.signer)
              .vote(proposalId, voteSupport, { gasLimit: 200000 });
            await voteTx.wait();
            console.log(
              c.green(
                `      ✅ ${p.name} voted ${
                  voteSupport === 1 ? "FOR" : "AGAINST"
                }`
              )
            );
            ...
            console.log(
              c.yellow(
                `      ⚠️  ${p.name} voting failed: ${error.message.slice(
                  0,
                  60
                )}`
              )
        ...
        if (finalProposal.success) {
          ...
          console.log(
            c.dim(
              `       Against: ${ethers.utils.formatEther(
                finalProposal.data.againstVotes
              )}`
            )
          );
          console.log(
            c.dim(
              `       Abstain: ${ethers.utils.formatEther(
                finalProposal.data.abstainVotes
              )}`
            )
          );
        }
      ...
      console.log(
        c.yellow(
          `      ⚠️  Governance proposal test not possible: ${error.message.slice(
            0,
            100
          )}`
        )
...
async function demoGovernanceSystem(ajoFactory, ajoId, participants) {
  ...
  console.log(
    c.yellow(
      "  (Detailed governance testing done in demoGovernanceViewFunctions)"
    )
  );
}
...
async function demoCollateralSystem(ajoFactory, ajoId) {
  ...
  console.log(
    c.yellow(
      "  (Detailed collateral testing done in demoCollateralViewFunctions)"
    )
  );
}
...
async function demoDefaultHandling(ajo, ajoCollateral, participants) {
  ...
  console.log(
    c.yellow("  (Seizable assets testing done in demoCollateralViewFunctions)")
  );
}

async function demoMultiTokenSupport(
  ajo,
  ajoPayments,
  usdc,
  whbar,
  ...
  console.log(
    c.yellow("  (Token config testing done in demoPaymentViewFunctions)")
  );
}
...
async function demoReputationSystem(ajo, participants) {
  ...
  console.log(
    c.yellow("  (Reputation testing done in demoMemberViewFunctions)")
  );
}
...
async function demoPaymentCycleInsights(ajo, ajoPayments) {
  ...
  console.log(
    c.yellow("  (Payment analytics done in demoPaymentViewFunctions)")
  );
}
...
async function demoEmergencyFeatures(ajo, participants) {
  ...
  console.log(
    c.yellow("  (Security metrics testing done in demoCoreViewFunctions)")
  );
}
...
async function verifyAdvancedCollateralFeatures(ajoCollateral, participants) {
  ...
  for (const scenario of testScenarios) {
    ...
    for (const pos of scenario.positions) {
      const guarantorPos = await safeViewCall(
        async () =>
          await ajoCollateral.calculateGuarantorPosition(
            pos,
      ...
      if (guarantorPos.success) {
        console.log(
          c.dim(`        Position ${pos} → Guarantor ${guarantorPos.data}`)
        );
      }
    }
  }
  ...
  for (const scenario of collateralScenarios) {
    console.log(
      c.dim(
        `\n      ${scenario.size} people, ${formatUSDC(
          scenario.payment
        )} monthly:`
      )
    );
    for (let pos = 1; pos <= Math.min(3, scenario.size); pos++) {
      ...
      if (required.success) {
        console.log(
          c.dim(`        Position ${pos}: ${formatUSDC(required.data)}`)
        );
      }
    }
  }
...
async function verifyMemberIndexing(ajoMembers, participants) {
  ...
  for (let i = 0; i < Math.min(5, participants.length); i++) {
    ...
    if (memberAddr.success) {
      console.log(
        c.dim(`      Index ${i}: ${memberAddr.data.slice(0, 10)}...`)
      );

      // Verify it matches expected participant
      const matchingParticipant = participants.find(
        (p) => p.address.toLowerCase() === memberAddr.data.toLowerCase()
      );
  ...
  for (let pos = 1; pos <= Math.min(5, participants.length); pos++) {
    ...
    if (queueAddr.success) {
      console.log(
        c.dim(`      Queue ${pos}: ${queueAddr.data.slice(0, 10)}...`)
      );
    }
  }
}
  ...
  if (totalPayouts.success && totalPayouts.data.gt(0)) {
    ...
    for (let i = 1; i <= Math.min(10, count); i++) {
      ...
      if (payout.success) {
        ...
        console.log(
          c.dim(`        Recipient: ${payout.data.recipient.slice(0, 10)}...`)
        );
...
async function verifyTokenConfiguration(ajo, ajoPayments) {
  ...
  for (let tokenId = 0; tokenId <= 1; tokenId++) {
    ...
    if (config.success) {
      ...
      const formatted =
        tokenId === 0
          ? formatUSDC(config.data.monthlyPayment)
          : formatHBAR(config.data.monthlyPayment);
      console.log(
        c.dim(
          `      ${tokenName}: ${formatted}, Active: ${config.data.isActive}`
        )
      );
    }
  }
...
async function verifyFactoryPagination(ajoFactory) {
  ...
  for (const pageSize of pageSizes) {
    ...
    if (result.success) {
      console.log(
        c.dim(
          `      Page size ${pageSize}: Got ${result.data.ajoInfos.length}, HasMore: ${result.data.hasMore}`
        )
      );
    }
  }
  ...
  if (totalCount > 2) {
    ...
    if (result.success) {
      console.log(
        c.dim(
          `      Starting from offset 1: Got ${result.data.ajoInfos.length} Ajos`
        )
      );
    }
  }
...
async function verifySeizableAssetsForAll(ajo, ajoCollateral, participants) {
  ...
  for (let i = 0; i < Math.min(5, participants.length); i++) {
    ...
    if (assets.success) {
      console.log(c.dim(`      ${p.name}:`));
      console.log(
        c.dim(`        Total: ${formatUSDC(assets.data.totalSeizable)}`)
      );
      console.log(
        c.dim(`        Collateral: ${formatUSDC(assets.data.collateralSeized)}`)
      );
      console.log(
        c.dim(`        Payments: ${formatUSDC(assets.data.paymentsSeized)}`)
      );
    }
  }
  ...
  for (let i = 0; i < Math.min(3, participants.length); i++) {
    ...
    if (assets.success) {
      console.log(
        c.dim(`      ${p.name}: Total ${formatUSDC(assets.data.totalSeizable)}`)
      );
    }
  }
}
...
async function runAdvancedDemo(
  ...
  try {
    ...
    console.log(
      c.green(
        "✅ Factory (9): getFactoryStats, getImplementations, getAllAjos, getAjosByCreator, getAjo, getAjoCore, ajoStatus, getAjoInitializationStatus"
      )
    );
    console.log(
      c.green(
        "✅ Core (7): getContractStats, getTokenConfig, getCollateralDemo, getMemberInfo, getQueueInfo, needsToPayThisCycle, calculateSeizableAssets"
      )
    );
    console.log(
      c.green(
        "✅ Members (13): getTotalActiveMembers, getActiveMembersList, isMember, getMember, getMemberInfo, getQueueInfo, getLockedCollateral, getQueuePosition, getGuarantorForPosition, getMemberAtIndex, getContractStats, activeMembersList, queuePositions"
      )
    );
    console.log(
      c.green(
        "✅ Collateral (4): getTotalCollateral, calculateRequiredCollateral, calculateGuarantorPosition, calculateSeizableAssets"
      )
    );
    console.log(
      c.green(
        "✅ Payments (14): getCurrentCycle, getNextPayoutPosition, getActivePaymentToken, getPenaltyRate, getTokenConfig (x2), getContractBalance (x2), isPayoutReady, calculatePayout, getNextRecipient, getTotalPayouts, getPayout, needsToPayThisCycle, getPendingPenalty"
      )
    );
    console.log(
      c.green("✅ Governance (3): getGovernanceSettings, getProposal, hasVoted")
    );
