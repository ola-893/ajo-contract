...
async function showPreJoiningState(
  ajo,
  ajoCollateral,
  ajoMembers,
  ajoPayments
  ...
  if (usdcConfig.success) {
    console.log(
      c.dim(
        `       Monthly Payment: ${formatUSDC(usdcConfig.data.monthlyPayment)}`
      )
    );
  ...
  if (hbarConfig.success) {
    console.log(
      c.dim(
        `       Monthly Payment: ${formatHBAR(hbarConfig.data.monthlyPayment)}`
      )
    );
  ...
  if (collateralDemo.success) {
    console.log(
      c.dim(
        `       Position 1: ${formatUSDC(collateralDemo.data.collaterals[0])}`
      )
    );
    console.log(
      c.dim(
        `       Position 5: ${formatUSDC(collateralDemo.data.collaterals[4])}`
      )
    );
    console.log(
      c.dim(
        `       Position 10: ${formatUSDC(collateralDemo.data.collaterals[9])}`
      )
    );
  }
  ...
  if (activeToken.success) {
    console.log(
      c.dim(`       Active Token: ${activeToken.data === 0 ? "USDC" : "HBAR"}`)
    );
  }
}
...
async function verifyJoiningResults(
  ajo,
  ajoCollateral,
  ajoMembers,
  participants
  ...
  if (totalCollateral.success) {
    console.log(
      c.dim(
        `       Total USDC Locked: ${formatUSDC(
          totalCollateral.data.totalUSDC
        )}`
      )
    );
    console.log(
      c.dim(
        `       Total HBAR Locked: ${formatHBAR(
          totalCollateral.data.totalHBAR
        )}`
      )
    );
  }
  ...
  if (stats.success) {
    ...
    console.log(
      c.dim(`       Current Queue Position: ${stats.data.currentQueuePosition}`)
    );
    console.log(
      c.dim(
  ...
  if (membersList.success) {
    console.log(
      c.dim(`       Active Members Array Length: ${membersList.data.length}`)
    );
  }
  ...
  for (let i = 0; i < Math.min(3, participants.length); i++) {
    ...
    if (memberInfo.success) {
      ...
      console.log(
        c.dim(
          `         Required Collateral: ${formatUSDC(
            memberInfo.data.memberInfo.requiredCollateral
          )}`
        )
      );
      console.log(
        c.dim(`         Guarantor: ${memberInfo.data.memberInfo.guarantor}`)
      );
      console.log(
        c.dim(
          `         Reputation: ${memberInfo.data.memberInfo.reputationScore}`
        )
      );
    }
    ...
    if (locked.success) {
      console.log(
        c.dim(`         Locked (from Members): ${formatUSDC(locked.data)}`)
      );
    }
  }
  ...
  for (let pos = 1; pos <= Math.min(3, participants.length); pos++) {
    ...
    if (address.success) {
      console.log(
        c.dim(`       Position ${pos}: ${address.data.slice(0, 10)}...`)
      );
    }
...
async function showPrePaymentState(ajo, ajoPayments, participants) {
  ...
  for (let i = 0; i < Math.min(5, participants.length); i++) {
    ...
    if (needsPay.success) {
      console.log(
        c.dim(
          `       ${p.name}: ${
            needsPay.data ? "NEEDS TO PAY" : "No payment needed"
          }`
        )
      );
    }
    ...
    if (penalty.success && penalty.data.gt(0)) {
      console.log(
        c.dim(`       ${p.name} Penalty: ${formatUSDC(penalty.data)}`)
      );
    }
  }
  ...
  if (nextRecipient.success) {
    console.log(
      c.dim(`       Next Recipient: ${nextRecipient.data.slice(0, 10)}...`)
    );
  }
  ...
  if (calculatedPayout.success) {
    console.log(
      c.dim(`       Expected Payout: ${formatUSDC(calculatedPayout.data)}`)
    );
  }
...
async function verifyPaymentResults(ajo, ajoPayments, participants) {
  ...
  if (totalPayouts.success) {
    ...
    if (totalPayouts.data.gt(0)) {
      ...
      if (latestPayout.success) {
        ...
        console.log(
          c.dim(
            `         Recipient: ${latestPayout.data.recipient.slice(0, 10)}...`
          )
        );
        console.log(
          c.dim(`         Amount: ${formatUSDC(latestPayout.data.amount)}`)
        );
  ...
  for (let i = 0; i < Math.min(3, participants.length); i++) {
    ...
    if (memberInfo.success) {
      ...
      console.log(
        c.dim(
          `         Total Paid: ${formatUSDC(
            memberInfo.data.memberInfo.totalPaid
          )}`
        )
      );
      console.log(
        c.dim(
          `         Last Payment Cycle: ${memberInfo.data.memberInfo.lastPaymentCycle}`
        )
      );
      console.log(
        c.dim(
          `         Has Received Payout: ${memberInfo.data.memberInfo.hasReceivedPayout}`
        )
      );
      console.log(
        c.dim(
          `         Default Count: ${memberInfo.data.memberInfo.defaultCount}`
        )
      );
    }
  }
...
async function showFactoryState(ajoFactory) {
  ...
  if (stats.success && stats.data.totalCreated.gt(0)) {
    ...
    if (ajoStatus.success) {
      console.log(
        c.dim(
          `       Ajo 1 Status: Exists=${ajoStatus.data.exists}, Active=${ajoStatus.data.isActive}`
        )
      );
    }
    ...
    if (coreAddress.success) {
      console.log(
        c.dim(`       Ajo 1 Core: ${coreAddress.data.slice(0, 10)}...`)
      );
    }
  }
}
...
async function showGovernanceState(ajoFactory, ajoId) {
  ...
  const ajoGovernance = await ethers.getContractAt(
    "AjoGovernance",
  ...
  if (settings.success) {
    console.log(
      c.dim(`       Proposal Threshold: ${settings.data.proposalThreshold}`)
    );
    console.log(c.dim(`       Voting Period: ${settings.data.votingPeriod}`));
    console.log(
      c.dim(`       Current Penalty Rate: ${settings.data.currentPenaltyRate}%`)
    );
    console.log(
    ...
    if (settings.data.totalProposals.gt(0)) {
      ...
      if (proposal.success) {
        ...
        console.log(
          c.dim(`         Against Votes: ${proposal.data.againstVotes}`)
        );
...
async function demonstrateJoiningWithVerification(
  ajo,
  ajoCollateral,
  ajoMembers,
  participants
  ...
  for (let i = 0; i < participants.length; i++) {
    ...
    try {
      console.log(
        c.dim(
          `  ${i + 1}/${participants.length}: ${participant.name} joining...`
        )
      );

      const joinTx = await ajo
        .connect(participant.signer)
        .joinAjo(0, { gasLimit: 800000 });
      ...
      console.log(
        c.green(
          `    ✅ SUCCESS! Locked: ${ethers.utils.formatUnits(
            actualCollateral,
            6
          )} USDC | Gas: ${receipt.gasUsed.toString()}`
        )
      );
    } catch (error) {
      console.log(
        c.red(`    ❌ ${participant.name}: ${error.reason || error.message}`)
      );
      joinResults.push({
        name: participant.name,
        position: participant.position,
...
async function demonstratePaymentCycleWithVerification(
  ajo,
  ajoPayments,
  participants
) {
  ...
  for (let i = 0; i < participants.length; i++) {
    ...
    try {
      ...
      const paymentTx = await ajo
        .connect(participant.signer)
        .processPayment({ gasLimit: 900000 });
      ...
      console.log(
        c.green(`    ✅ Payment processed | Gas: ${receipt.gasUsed.toString()}`)
      );
    } catch (error) {
      console.log(
        c.red(`    ❌ ${participant.name} payment failed: ${error.message}`)
      );
      paymentResults.push({
        name: participant.name,
        success: false,
  ...
  try {
    ...
    console.log(
      c.green(`    ✅ Payout distributed | Gas: ${receipt.gasUsed.toString()}`)
    );
  } catch (error) {
