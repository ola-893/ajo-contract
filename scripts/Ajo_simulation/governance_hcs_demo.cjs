...
function convertHcsTopicIdToHedera(bytes32TopicId) {
  try {
    // Check if already in Hedera format (0.0.xxxxx)
    ...
    // Convert from bytes32 hex format
...
async function runGovernanceDemo(ajoGovernance, participantsInput, ajoInfo) {
  ...
  const { votes, hcsReceipts } = await testRealHcsVoteSubmission(hederaClient, ajoGovernance, proposalId, members, ajoInfo.hcsTopicId);
  await sleep(2000);
